...
import { GaugeAPY } from "./module/types/qoracle/gauge_apy"
...
export { GaugeAPY, Params, PoolInfo, PoolMetrics, PoolPosition, PoolRanking, PoolSpotPrice };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						GaugeAPY: getStructure(GaugeAPY.fromPartial({})),
	...
	actions: {
		...
		async QueryPoolPosition({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryPoolPosition', payload: { options: { all }, params: {...key},query }})
		...
		async QueryPoolPositionAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryPoolPositionAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'PoolPositionAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryPoolPositionAll', payload: { options: { all }, params: {...key},query }})
		...
		async QueryPoolRanking({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryPoolRanking', payload: { options: { all }, params: {...key},query }})
		...
		async QueryPoolSpotPrice({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryPoolSpotPrice', payload: { options: { all }, params: {...key},query }})
		...
		async QueryPoolSpotPriceAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryPoolSpotPriceAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'PoolSpotPriceAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryPoolSpotPriceAll', payload: { options: { all }, params: {...key},query }})
		...
		async QueryPoolInfo({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryPoolInfo', payload: { options: { all }, params: {...key},query }})
		...
		async QueryPoolInfoAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryPoolInfoAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'PoolInfoAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryPoolInfoAll', payload: { options: { all }, params: {...key},query }})
