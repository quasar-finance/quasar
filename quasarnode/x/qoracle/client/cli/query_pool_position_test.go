...
func networkWithPoolPositionObjects(t *testing.T) (*network.Network, types.PoolPosition) {
	t.Helper()
	...
	state.PoolPosition = poolPosition
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), *state.PoolPosition
}

func TestShowPoolPosition(t *testing.T) {
	net, obj := networkWithPoolPositionObjects(t)
