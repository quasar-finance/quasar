...
use crate::state::{MigrationStatus, MAIN_POSITION_ID, MIGRATION_STATUS, POOL_CONFIG, POSITIONS};
...
pub fn execute_autocompound(
    ...
    let position_state = POSITIONS.load(deps.storage, MAIN_POSITION_ID.load(deps.storage)?)?;
