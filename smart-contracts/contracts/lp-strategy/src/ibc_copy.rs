...
pub fn handle_succesful_ack(
    ...
    match kind {
        ...
        IbcMsgKind::Transfer {
            ref pending,
            amount,
        } => match handle_transfer_ack(deps.storage, env, pending.clone(), amount) {
        ...
        IbcMsgKind::Ica(ref mut ica_kind) => {
            match handle_ica_ack(deps.storage, env, ack_bin, ica_kind) {
                Ok(response) => Ok(response),
                Err(err) => {
                    TRAPS.save(
                        deps.storage,
...
pub fn handle_ica_ack(
    ...
    match ica_kind {
        IcaMessages::JoinSwapExternAmountIn(data) => {
            handle_join_swap_extern_amount_in_ack(storage, &env, data, ack_bin)
        }
        // TODO move the lock_tokens ack handling to a seperate function  (DONE)
...
fn handle_join_swap_extern_amount_in_ack(
    storage: &mut dyn Storage,
    env: &Env,
...
mod test {
    ...
    fn handle_transfer_ack_works() {
        ...
        // mock deposit
        let ongoing_deposit = OngoingDeposit {
        ...
        let pending = PendingBond {
            bonds: vec![ongoing_deposit],
        };

        let total_amount = Uint128::new(100);

        let res = handle_transfer_ack(
            deps.as_mut().storage,
            env.clone(),
            pending.clone(),
            total_amount,
        ...
        let expected = do_ibc_join_pool_swap_extern_amount_in(
            deps.as_mut().storage,
            env.clone(),
            config.pool_id,
            config.base_denom.clone(),
        ...
        assert_eq!(res.unwrap().messages[0], expected);
    }

    #[test]
    fn handle_start_unbonding_ack_works() {
        let mut deps = mock_dependencies();
        default_setup(deps.as_mut().storage).unwrap();
        let env = mock_env();
        let total_amount = Uint128::new(100);
