...
impl Querier for QuasarQuerier {
    fn raw_query(&self, bin_request: &[u8]) -> cosmwasm_std::QuerierResult {
        let request: QueryRequest<Empty> = from_json_binary(&Binary::from(bin_request)).unwrap();
        match request {
            QueryRequest::Wasm(wasm_query) => match wasm_query {
                cosmwasm_std::WasmQuery::Smart { contract_addr, msg } => {
                    let primitive_query = from_json_binary::<lp_strategy::msg::QueryMsg>(&msg).unwrap();
...
fn proper_initialization() {
    ...
    }) = &res.messages[0].msg
    {
        ...
        let msg: vault_rewards::msg::InstantiateMsg = from_json_binary(msg).unwrap();
        assert_eq!(
...
fn test_get_deposit_amount_weights() {
    ...
    let investment_response: InvestmentResponse = from_json_binary(&query_res).unwrap();
...
fn proper_bond() {
    ...
    }) = &res.messages[0].msg
    {
        ...
        if let lp_strategy::msg::ExecuteMsg::Bond { id } = from_json_binary(msg).unwrap() {
            assert_eq!(id, "1")
        } else {
...
fn proper_bond_response_callback_single_token() {
    ...
    let balance: BalanceResponse = from_json_binary(&balance_res).unwrap();
...
fn proper_unbond() {
    ...
    }) = &claim_res.messages[0].msg
    {
        assert_eq!(contract_addr, "quasar123");
        assert!(funds.is_empty());
        if let lp_strategy::msg::ExecuteMsg::Unbond { id } = from_json_binary(msg).unwrap() {
            assert_eq!(id, "2");
        } else {
