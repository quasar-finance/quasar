...
use crate::{
    state::{BondingStub, DEBUG_TOOL, DEPOSIT_STATE, INVESTMENT, PENDING_BOND_IDS, TOTAL_SUPPLY},
    ContractError,
};
...
pub fn on_bond(
    ...
    // rewrite the above in a way that works in rust
    DEBUG_TOOL.save(
        deps.storage,
        &format!(
            "We hit on_bond with share_amount: {} bond_id: {} from: {}",
            share_amount, bond_id, info.sender
        ),
    )?;
