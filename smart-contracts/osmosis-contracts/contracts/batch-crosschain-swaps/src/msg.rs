...
pub enum ExecuteMsg {
    ...
    /// Execute a one-to-many swap and forward it to the receiver address on the specified ibc channel
    BatchOsmosisSwap {
        /// The final denoms to be received (as represented on osmosis)
        output_denoms: Vec<String>,
        /// The weights of all the output denoms to be received
        output_weights: Vec<Decimal>,
        /// The receiver of the IBC packet to be sent after the swap
        receiver: String,
        /// Slippage for all swaps
        slippage: Slippage,
        /// IBC packets can contain an optional memo. If a sender wants the sent
        /// packet to include a memo, this is the field where they can specify
        /// it. If provided, the memo is expected to be a valid JSON object
