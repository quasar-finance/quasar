...
pub fn unwrap_or_swap_and_forward_batch(
    ...
    let submsgs = output_denoms
        ...
        .map(|(output_denom, weight)| {
            let this_normed_weight = weight.checked_div(total_weight)?;
            let this_swap_coin = Coin {
                denom: swap_coin.denom.clone(),
                amount: (Uint128::from(swap_coin.amount.u128()).checked_multiply_ratio(
                    this_normed_weight.numerator(),
                    this_normed_weight.denominator(),
                ))?,
            };
