...
func ValidateUserSpecifiedPoolAssets(assets []PoolAsset) error {
	...
	if len(assets) < 2 {
		return gammtypes.ErrTooFewPoolAssets
	}

	// TODO: Add the limit of binding token to the pool params?
	if len(assets) > 8 {
		return sdkerrors.Wrapf(gammtypes.ErrTooManyPoolAssets, "%d", len(assets))
	}
	...
	for _, asset := range assets {
		...
		if _, exists := assetExistsMap[asset.Token.Denom]; exists {
			return sdkerrors.Wrapf(gammtypes.ErrTooFewPoolAssets, "pool asset %s already exists", asset.Token.Denom)
		}
