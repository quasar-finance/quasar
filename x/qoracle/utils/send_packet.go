...
func SendPacket(
	...
	if !found {
		return channeltypes.Packet{}, errors.Wrapf(
	...
	if !found {
		return channeltypes.Packet{}, errors.Wrapf(channeltypes.ErrSequenceSendNotFound, "failed to retrieve next sequence send for channel %s on port %s", sourceChannel, sourcePort)
	}

	chanCap, ok := scopedKeeper.GetCapability(ctx, host.ChannelCapabilityPath(sourcePort, sourceChannel))
	if !ok {
		return channeltypes.Packet{}, errors.Wrap(channeltypes.ErrChannelCapabilityNotFound,
...
func convertRelativeToAbsoluteTimeout(
	...
	if !ok {
		return clienttypes.ZeroHeight(), 0, errors.Wrapf(sdkerrors.ErrInvalidHeight, "invalid height type. expected type: %T, got: %T",
