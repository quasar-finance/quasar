...
func (s *KeeperTestSuite) SetupTest() {
	...
	fundAccsAmount := sdk.NewCoins(sdk.NewCoin(apptesting.TestDenom, apptesting.SecondaryAmount))
	fundAccsAmount2 := sdk.NewCoins(sdk.NewCoin("uqsr", apptesting.SecondaryAmount))
	for _, acc := range s.TestAccs {
		s.FundAcc(acc, fundAccsAmount)
		s.FundAcc(acc, fundAccsAmount2)
	}
	s.contractKeeper = wasmkeeper.NewGovPermissionKeeper(s.App.WasmKeeper)
	s.queryClient = types.NewQueryClient(s.QueryHelper)
	s.msgServer = keeper.NewMsgServerImpl(s.App.TfKeeper)
...
func (s *KeeperTestSuite) TestCreateModuleAccount() {
	...
	app.TfKeeper.CreateModuleAccount(s.Ctx)
