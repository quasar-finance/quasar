...
func (s *KeeperTestSuite) TestAdminMsgs() {
	...
	govModuleAccount := s.App.AccountKeeper.GetModuleAccount(s.Ctx, govtypes.ModuleName)
	_, err = s.msgServer.ForceTransfer(s.Ctx, types.NewMsgForceTransfer(govModuleAccount.GetAddress().String(), sdk.NewInt64Coin(s.defaultDenom, 5), s.TestAccs[1].String(), s.TestAccs[0].String()))
	addr1bal -= 5
	addr0bal += 5
	s.Require().NoError(err)
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			forceTransferMsg: *types.NewMsgForceTransfer(
				govModuleAccount.GetAddress().String(),
				sdk.NewInt64Coin(s.defaultDenom, 10),
				s.TestAccs[1].String(),
				s.TestAccs[2].String(),
			),
		...
		{
			desc: "forceTransfer is not by the gov module account",
			forceTransferMsg: *types.NewMsgForceTransfer(
				s.TestAccs[1].String(),
				sdk.NewInt64Coin(s.defaultDenom, 10),
				s.TestAccs[1].String(),
