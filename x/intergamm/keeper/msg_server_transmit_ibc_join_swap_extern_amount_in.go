...
func (ms msgServer) TransmitIbcJoinSwapExternAmountIn(goCtx context.Context, in *types.MsgTransmitIbcJoinSwapExternAmountIn) (*types.MsgTransmitIbcJoinSwapExternAmountInResponse, error) {
	...
	seq, channel, err := ms.k.TransmitIbcJoinSwapExternAmountIn(ctx, in.GetCreator(), in.GetConnectionId(), in.TimeoutTimestamp, in.PoolId, in.GetTokenIn(), sdk.NewInt(in.GetShareOutMinAmount()))
	if err != nil {
		return nil, err
	}
