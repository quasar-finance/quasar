...
func (ms msgServer) TransmitIbcExitSwapExternAmountOut(goCtx context.Context, out *types.MsgTransmitIbcExitSwapExternAmountOut) (*types.MsgTransmitIbcExitSwapExternAmountOutResponse, error) {
	...
	seq, err := ms.k.TransmitIbcExitSwapExternAmountOut(ctx, out.GetCreator(), out.GetConnectionId(), out.TimeoutTimestamp, out.PoolId, out.GetTokenOutMins(), sdk.NewInt(out.GetShareInAmount()))
	if err != nil {
		return nil, err
	}
