...
func CmdForwardTransferIbcTokens() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "forward-transfer-ibc-tokens [src-port] [src-channel] [fwd-transfer-port] [fwd-transfer-channel] [intermediate-receiver] [receiver] [amount]",
		Short: "Broadcast message forwardTransferIbcTokens",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			srcPort := args[0]
			srcChannel := args[1]
			fwdTransferPort := args[2]
			fwdTransferChannel := args[3]
			intermediateReceiver := args[4]
			receiver := args[5]
			...
			msg := types.NewMsgForwardTransferIbcTokens(
