...
quasarnoded tx wasm migrate $PRIM1 $PRIM_CODE_ID "{\"config\": $CONFIG1}" --from laurens-test --keyring-backend test -b block $TXFLAG 
quasarnoded tx wasm migrate $PRIM2 $PRIM_CODE_ID "{\"config\": $CONFIG2}" --from laurens-test --keyring-backend test -b block $TXFLAG 
quasarnoded tx wasm migrate $PRIM3 $PRIM_CODE_ID "{\"config\": $CONFIG3}" --from laurens-test --keyring-backend test -b block $TXFLAG 
