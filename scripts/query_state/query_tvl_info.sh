...
if [ ! -f log_tvl_info.txt ] || ! grep -q "timestamp;base_denom;ica_address;lp_denom;d_unlocked_shares;locked_shares;w_unlocked_shares;quote_denom" log_tvl_info.txt; then
    echo "timestamp;base_denom;ica_address;lp_denom;d_unlocked_shares;locked_shares;w_unlocked_shares;quote_denom" > log_tvl_info.txt
fi
...
for primitive in $(echo "${OUTPUT}" | jq -c '.data.primitives[]'); do
    ...
    echo "$(date -u +"%Y-%m-%dT%H:%M:%S");$BASE_DENOM;$ICA_ADDRESS;$LP_DENOM;$D_UNLOCKED_SHARES;$LOCKED_SHARES;$W_UNLOCKED_SHARESs;$QUOTE_DENOM" >> log_tvl_info.txt
done