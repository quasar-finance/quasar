...
RES=$(quasarnoded tx wasm store artifacts/lp_strategy.wasm --from test-laurens -y --output json -b block $TXFLAG) 
CODE_ID=$(echo $RES | jq -r '.logs[0].events[-1].attributes[0].value') 
echo "Got CODE_ID = $CODE_ID"

echo "Deploying contract"
# swallow output
OUT1=$(quasarnoded tx wasm instantiate $CODE_ID "$INIT1" --from test-laurens  --label "primitive-1" --gas-prices 10$FEE_DENOM --gas auto --gas-adjustment 1.3 -b block -y --no-admin $NODE --chain-id $CHAIN_ID)
...
OUT2=$(quasarnoded tx wasm instantiate $CODE_ID "$INIT2" --from test-laurens --label "primitive-2" --gas-prices 10$FEE_DENOM --gas auto --gas-adjustment 1.3 -b block -y --no-admin $NODE --chain-id $CHAIN_ID)
...
OUT3=$(quasarnoded tx wasm instantiate $CODE_ID "$INIT3" --from test-laurens --label "primitive-3" --gas-prices 10$FEE_DENOM --gas auto --gas-adjustment 1.3 -b block -y --no-admin $NODE --chain-id $CHAIN_ID)
...
RES=$(quasarnoded tx wasm store artifacts/basic_vault.wasm --from test-laurens -y --output json -b block $TXFLAG)
...
# swallow output
OUT=$(quasarnoded tx wasm instantiate $VAULT_CODE_ID "$VAULT_INIT" --from test-laurens --label "vault-contract" --gas-prices 10$FEE_DENOM --gas auto --gas-adjustment 1.3 -b block -y --no-admin $NODE --chain-id $CHAIN_ID)
