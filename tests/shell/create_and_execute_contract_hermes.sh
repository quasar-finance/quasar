...
docker exec localenv-hermes-1  hermes create channel --a-chain quasar --a-connection connection-0 --a-port wasm.$ADDR1 --b-port icahost --order ordered --chan-version '{"version":"ics27-1","encoding":"proto3","tx_type":"sdk_multi_msg","controller_connection_id":"connection-0","host_connection_id":"connection-0"}'
docker exec localenv-hermes-1  hermes create channel --a-chain quasar --a-connection connection-0 --a-port wasm.$ADDR1 --b-port icqhost --order unordered --chan-version icq-1
