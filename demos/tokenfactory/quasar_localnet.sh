...
ALICE_GENESIS_COINS=20000token,200000000stake,10000000000uqsr
BOB_GENESIS_COINS=10000token,100000000stake,10000000000uqsr
...
$BINARY add-genesis-account $($BINARY keys show alice --keyring-backend test -a) $ALICE_GENESIS_COINS --home $HOME_QSR  --keyring-backend test
$BINARY add-genesis-account $($BINARY keys show bob --keyring-backend test -a) $BOB_GENESIS_COINS --home $HOME_QSR --keyring-backend test
$BINARY add-genesis-account $($BINARY keys show user1 --keyring-backend test -a) $USER_1_GENESIS_COINS --home $HOME_QSR  --keyring-backend test
$BINARY add-genesis-account $($BINARY keys show user2 --keyring-backend test -a) $USER_2_GENESIS_COINS --home $HOME_QSR  --keyring-backend test
$BINARY add-genesis-account $($BINARY keys show relayer_acc --keyring-backend test -a) $RELAYER_ACC_GENESIS_COINS --home $HOME_QSR --keyring-backend test
$BINARY gentx alice 100000000uqsr --chain-id $CHAIN_ID --keyring-backend test --home $HOME_QSR --keyring-backend test
...
cat $HOME_QSR/config/genesis_original.json |
  ...
  jq '.app_state.gov.voting_params.voting_period="600s"' |
