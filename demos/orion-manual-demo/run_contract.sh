...
echo $NODE
INIT="{}"
MSG='{"register_interchain_account":{"connection_id":"1"}}'
...
RES=$(quasard tx wasm store artifacts/intergamm_bindings_test.wasm --from alice -y --output json -b block $TXFLAG)
...
quasard tx wasm execute $ADDR "$MSG" --from alice --gas-prices 10$FEE_DENOM --gas auto --gas-adjustment 1.3 $NODE --chain-id $CHAIN_ID
