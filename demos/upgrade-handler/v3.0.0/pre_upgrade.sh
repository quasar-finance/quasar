...
do
  ...
  rly transact flush
  sleep $SLEEP_DURATION
  quasarnoded query bank balances $OSMOSIS_RECEIVER

  # Quasar to Osmosis transfer
  echo "Transferring from Quasar to Osmosis"
  quasarnoded tx ibc-transfer transfer transfer $QUASAR_CHANNEL $QUASAR_RECEIVER ${QUASAR_AMOUNT}${QUASAR_DENOM} --from $QUASAR_SENDER --keyring-backend test --home $QUASAR_HOME --chain-id $QUASAR_CHAIN_ID -y --gas-prices $GAS_PRICES --node $QUASAR_NODE
  sleep $SLEEP_DURATION
  rly transact flush
  sleep $SLEEP_DURATION
