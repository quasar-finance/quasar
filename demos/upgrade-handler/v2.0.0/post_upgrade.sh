...
QORACLE_PARAMS=$(quasard q qoracle params --output json --node $NODE)
...
QTRANSFER_PARAMS=$(quasard q qtransfer params --output json --node $NODE)
...
QVESTING_PARAMS=$(quasard q qvesting params --output json --node $NODE)
...
# Check account info
ACCOUNT_INFO=$(quasard q account quasar1sqlsc5024sszglyh7pswk5hfpc5xtl77gqjwec --output json --node $NODE)
...
# Check evidence
EVIDENCE=$(quasard q evidence --height 21 --output json --node $NODE)
...
execute "quasard tx wasm execute $OLD_CONTRACT_ADDRESS '$CONTRACT_EXECUTE_MSG' --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y"
...
execute "quasard tx bank send $SENDER_ADDRESS $RECEIVER_ADDRESS 10000000$DENOM --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y"
sleep $SLEEP_DURATION
execute "quasard query bank balances $RECEIVER_ADDRESS --node $NODE"

# Staking
echo "2. Staking"
execute "quasard tx staking delegate $VALIDATOR 1000000$DENOM --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y"
sleep $SLEEP_DURATION
execute "quasard query staking delegations $SENDER_ADDRESS --node $NODE"

# Claim Rewards
echo "3. Claim Rewards"
execute "quasard tx distribution withdraw-rewards $VALIDATOR --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y"
sleep $SLEEP_DURATION
execute "quasard query distribution rewards $SENDER_ADDRESS $VALIDATOR --node $NODE"

# Upload Contract
echo "4. Upload Contract"
execute "quasard tx wasm store $CONTRACT_WASM_PATH --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y --gas 2000000"
sleep $SLEEP_DURATION
...
execute "quasard tx wasm instantiate $CODE_ID '$CONTRACT_INIT_MSG' --from $SENDER --label '$CONTRACT_LABEL' --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y --admin $SENDER_ADDRESS"
sleep $SLEEP_DURATION
...
CONTRACT_EXECUTE_MSG='{"admin": {"add_users": {"users": [{"address": "quasar1sqlsc5024sszglyh7pswk5hfpc5xtl77gqjwec", "amount": "2500000000"}]}}}'
execute "quasard tx wasm execute $CONTRACT_ADDRESS '$CONTRACT_EXECUTE_MSG' --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y"
sleep $SLEEP_DURATION
...
execute "quasard tx gov submit-legacy-proposal --title '$PROPOSAL_TITLE' --description '$PROPOSAL_DESCRIPTION' --type '$PROPOSAL_TYPE' --deposit $PROPOSAL_DEPOSIT --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y"
sleep $SLEEP_DURATION
...
execute "quasard tx gov deposit $PROPOSAL_ID 5000000$DENOM --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y"
sleep $SLEEP_DURATION

# Vote on the Proposal
echo "10. Vote on the Proposal"
execute "quasard tx gov vote $PROPOSAL_ID yes --from $SENDER --chain-id $CHAIN_ID --gas-prices $GAS_PRICES --keyring-backend $KEYRING_BACKEND --home $HOME_DIR -y"
sleep $SLEEP_DURATION
...
do
  ...
  INITIAL_BALANCE=$(quasard query bank balances $RECEIVER_ADDRESS --output json --node $NODE | jq -r --arg denom "$QUASAR_DENOM" '.balances[] | select(.denom==$denom) | .amount')
  ...
  sleep $SLEEP_DURATION
  FINAL_BALANCE=$(quasard query bank balances $RECEIVER_ADDRESS --output json --node $NODE | jq -r --arg denom "$QUASAR_DENOM" '.balances[] | select(.denom==$denom) | .amount')
  check_greater "Balance" "$FINAL_BALANCE" "$INITIAL_BALANCE"

  REWARDS=$(quasard query distribution rewards $SENDER_ADDRESS $VALIDATOR --node $NODE --output json)
  ...
  INITIAL_STAKING=$(quasard query staking delegations $SENDER_ADDRESS --node $NODE --output json)
  ...
  sleep $SLEEP_DURATION
  POST_STAKING=$(quasard query staking delegations $SENDER_ADDRESS --node $NODE --output json)
