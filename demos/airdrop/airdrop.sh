...
quasard tx wasm execute $ADDR1 '{"admin": {"add_users": {"users": [{"address": "quasar1sqlsc5024sszglyh7pswk5hfpc5xtl77gqjwec", "amount": "2500000000"}, {"address": "quasar1828z63g9wp3qwyn4p64adc3ungsv56ux5aacmu", "amount": "2500000000"}, {"address": "quasar1zaavvzxez0elundtn32qnk9lkm8kmcszvnk6zf", "amount": "2500000000"}, {"address": "quasar185fflsvwrz0cx46w6qada7mdy92m6kx4xruj7p", "amount": "2500000000"}]}}}' --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block

quasard q wasm contract-state smart $ADDR1 '{"contract_state_query":{}}'

echo "Set alice amount to a higher amount so that it overflows and it should fail"
quasard tx wasm execute $ADDR1 '{"admin": {"set_users": {"users": [{"address": "quasar1sqlsc5024sszglyh7pswk5hfpc5xtl77gqjwec", "amount": "4500000000"}]}}}' --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block

echo "Update set new users to update airdrop amount for alice and bob and it should work"
quasard tx wasm execute $ADDR1 '{"admin": {"set_users": {"users": [{"address": "quasar1sqlsc5024sszglyh7pswk5hfpc5xtl77gqjwec", "amount": "1500000000"}, {"address": "quasar1828z63g9wp3qwyn4p64adc3ungsv56ux5aacmu", "amount": "3500000000"}]}}}' --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block

quasard q wasm contract-state smart $ADDR1 '{"contract_state_query":{}}'

echo "Remove alice from the airdrop eligibility"
quasard tx wasm execute $ADDR1 '{"admin": {"remove_users":["quasar1sqlsc5024sszglyh7pswk5hfpc5xtl77gqjwec"]}}' --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block

quasard q wasm contract-state smart $ADDR1 '{"contract_state_query":{}}'

echo "Add alcie to the airdrop eligibility"
quasard tx wasm execute $ADDR1 '{"admin": {"add_users": {"users": [{"address": "quasar1sqlsc5024sszglyh7pswk5hfpc5xtl77gqjwec", "amount": "1500000000"}]}}}' --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block

echo "funding contract account"
quasard tx bank send $ACCOUNT_ADDRESS $ADDR1 11000000000uqsr --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block

quasard q bank balances $ADDR1

AIRDROP_START_HEIGHT=15
AIRDROP_END_HEIGHT=25
echo "starting airdrop (but it will start form the given height in future)"
quasard tx wasm execute $ADDR1 '{ "admin" :{"update_airdrop_config": {"airdrop_amount": "11000000000","airdrop_asset": {"native": "uqsr"},"airdrop_title":"Test Title","airdrop_description": "Test description","end_height": '$AIRDROP_END_HEIGHT',"start_height": '$AIRDROP_START_HEIGHT',"total_claimed": "0"}}}' --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block
...
quasard query wasm contract-state smart $ADDR1 '{"airdrop_config_query":{}}'
quasard tx wasm execute $ADDR1 '{"claim_airdrop":[]}' --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block
quasard tx wasm execute $ADDR1 '{"claim_airdrop":[]}' --from bob --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block

quasard query wasm contract-state smart $ADDR1 '{"airdrop_config_query":{}}'
quasard tx wasm execute $ADDR1 '{"claim_airdrop":[]}' --from user1 --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block
quasard tx wasm execute $ADDR1 '{"claim_airdrop":[]}' --from user2 --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block
...
quasard q bank balances $ADDR1
quasard tx wasm execute $ADDR1 '{"admin": {"withdraw_funds":"quasar1sqlsc5024sszglyh7pswk5hfpc5xtl77gqjwec"}}' --from $ACCOUNT_NAME --keyring-backend test -y --output json --chain-id $CHAIN_ID --fees 10000uqsr --gas 7000000 -b block
quasard q bank balances $ADDR1